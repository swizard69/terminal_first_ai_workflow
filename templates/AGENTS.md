...
1. Run `scripts/ai-start` (or use global `ai-task` — runs ai-start for you).
2. Read files listed by `scripts/ai-context`.
3. Implement; run focused tests.
4. Update snapshot (`## Done`, `## Last Validation`).
5. End with `scripts/ai-ship -m "type(scope): summary"` (test + commit + push).
   Optional: `--mr` to create GitLab MR if none exists; merge stays human.
   Optional: `scripts/ai-finish` for checklist / index only.
